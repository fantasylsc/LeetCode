    ...
    def hotelRoom(self, s):
        ...
        for room in s:
            if room[0] == '-':
                ...
                dic[room[1:]] += 1
                if dic[room[1:]] > curr_max:
                    curr_max = dic[room[1:]]
                    res = room[1:]
                elif dic[room[1:]] == curr_max:
                    if room[2] < res[1]:
                        res = room[1:]
        return res
...
class Test(unittest.TestCase):
    def test1(self):
        ...
        expected = "1A"
        sol = Solution()
        actual = sol.hotelRoom(s)
        self.assertEqual(actual, expected)
    def test2(self):
        s = ["+1A", "+3E", "-1A", "+4F", "+1A", "-3E", "+3E", "-3E", "+3E"]
        expected = "3E"
        sol = Solution()
        actual = sol.hotelRoom(s)
        self.assertEqual(actual, expected)
    def test3(self):
        s = ["+1A", "+3E", "-1A", "+4F", "+1A", "-3E", "+3E", "-3E", "+3E", "-1A", "+1A"]
        expected = "1A"
        sol = Solution()
        actual = sol.hotelRoom(s)
        self.assertEqual(actual, expected)
